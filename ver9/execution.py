...
from .models import AllocationDecision, ExecutionResult
...
class ExecutionSummary:
    ...
    filled_orders: int
    rejected_orders: int
    fills: list[ExecutionResult] = field(default_factory=list)
    ...
    def as_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        payload["fills"] = [serialize_model(fill) for fill in self.fills]
...
def _build_telemetry(
...
) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any], float, float, float]:
    ...
    for order in orders:
        ...
        fill_price = fill.average_fill_price if fill else None
        ...
        if accepted:
            ...
            rejection_reason = next(
                (
                    r.get("reason")
                    for r in rejections
                    if str(r.get("strategy_id") or "") == order.strategy_id
                ),
                "unfilled",
            )
        ...
        per_strategy_stats[strategy_id] = {
            ...
            "execution_quality_score": round(
                max(
                    0.0,
                    (fill_rate * 0.55)
                    + ((1.0 - min(abs(avg_slip) / 250.0, 1.0)) * 0.45),
                ),
                6,
            ),
        }
    ...
    execution_quality_score = round(
        max(
            ...
            min(
                1.0,
                (fill_rate * 0.5)
                + ((1.0 - min(abs(avg_slippage) / 250.0, 1.0)) * 0.35)
                + ((1.0 - (len(rejections) / len(orders) if orders else 0.0)) * 0.15),
            ),
        ),
        6,
    )
...
class PaperBroker:
    ...
    def submit(self, orders: list[OrderRequest]) -> ExecutionSummary:
        ...
        for order in orders:
            ratio = _stable_ratio(
                f"{order.strategy_id}:{order.symbol}:{order.side}:{order.quantity}"
            )
            ...
            fill = ExecutionResult(
                ...
                symbol=order.symbol,
                side=order.side,
                requested_qty=round(order.quantity, 8),
                filled_qty=round(order.quantity, 8),
                requested_price=float(order.price or fill_price),
                average_fill_price=round(fill_price, 4),
                slippage_bps=round(_slippage_bps(order.price, fill_price), 4),
                latency_ms=int(50 + (ratio * 150)),
                status="filled",
                rejection_reason=None,
        ...
        (
            ...
            avg_slippage_bps,
            exec_quality,
        ) = _build_telemetry(orders, fills, rejections)
...
def build_orders_from_allocations(
    allocations: list[AllocationDecision | dict[str, Any]],
    *,
    capital: float,
) -> list[OrderRequest]:
    orders: list[OrderRequest] = []
    ...
    for allocation in allocations:
        if isinstance(allocation, AllocationDecision):
            pct = allocation.weight
            symbol = allocation.symbol
            strategy_id = allocation.strategy_id
            family = str(allocation.metadata.get("family") or "")
        else:
        ...
        orders.append(
            OrderRequest(
                ...
                symbol=symbol,
                side="buy"
                if family in {"mean_reversion", "volatility_compression"}
                else "buy",
...
def execute_allocations(
    ...
    *,
    ...
    return broker.submit(
        build_orders_from_allocations(allocations, capital=capital)
    )
