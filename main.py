...
def cmd_evolve(args: argparse.Namespace) -> None:
    ...
    for row in generated:
        validation = validate_candidate(
            ...
            trials=args.perturbation_trials,
        )
    ...
    deployable = [row for row in promoted if row.get("status") in {"validated", "probationary", "deployable"}]
...
def cmd_portfolio_probationary(args: argparse.Namespace) -> None:
    rows = _registry_candidates(args)
    portfolio = probationary_portfolio(rows, max_positions=args.max_positions, min_positions=args.min_positions)
    basket = basket_summary(rows, max_positions=args.max_positions, min_positions=args.min_positions, soft_fill=True)
...
def cmd_distributed(args: argparse.Namespace) -> None:
    coordinator = DistributedEvolutionCoordinator(worker_count=args.worker_count, batch_size=args.batch_size)
...
def cmd_risk(args: argparse.Namespace) -> None:
    ...
    drift = monitor.evaluate_drift(
        ...
        live_metrics={"return_pct": args.live_return, "max_drawdown_pct": args.live_drawdown},
        expected_metrics={"return_pct": args.expected_return, "max_drawdown_pct": args.expected_drawdown},
    )
